...
LLInt SparseHamiltonian::mod_(LLInt a, LLInt b)
{
  return (a % b + b) % b;
}
...
/*******************************************************************************/
// Determines the distribution among nodes for the basis
/*******************************************************************************/
void SparseHamiltonian::node_distribution(PetscInt &nlocal, PetscInt &start, PetscInt &end)
{
  nlocal = -1;
  start = -1;
  end = -1;
  ...
  if(node_rank_ == 0){
    ...
    PetscInt node_0_id = mpirank_ / node_size_; 

    nlocal = basis_size_ / nodes;
    PetscInt rest = basis_size_ % nodes;

    if(rest && (node_0_id < rest)) nlocal++;

    start = node_0_id * nlocal;
    if(rest && (node_0_id >= rest)) start += rest;
...
/*******************************************************************************/
void SparseHamiltonian::get_random_initial_pick(LLInt &pick_ind, LLInt *int_basis, 
    PetscInt start, PetscInt end, bool wtime, bool verbose)
{
...
void SparseHamiltonian::neel_initial_vec(Vec &initial, LLInt neel_index)
{
  ...
  if(neel_index != -1) VecSetValue(initial, neel_index, 1.0, INSERT_VALUES);
...
void SparseHamiltonian::determine_allocation_details_(LLInt *int_basis, 
    std::vector<LLInt> &cont, std::vector<LLInt> &st, const PetscInt m, PetscInt start, 
        PetscInt end, PetscInt node_nlocal, PetscInt node_start, PetscInt node_end,
            PetscInt *diag, PetscInt *off)
{
  ...
  for(PetscInt state = start; state < end; ++state){
    ...
    PetscInt basis_ind;
    node_rank_ ? basis_ind = state - start : basis_ind = state - node_start;
      ...
      if(bitset[site] == 1){
        ...
        // Otherwise do a swap
          ...
          if(node_rank_){
            ...
            match_ind1 = binsearch(int_basis, node_nlocal, new_int1);
            if(match_ind1 == -1){
              cont.push_back(new_int1);
              st.push_back(state);
              continue;
      ...
      // Case 2: There's no particle in this site
        ...
        if(bitset[next_site0] == 1){
          ...
          if(node_rank_){
            ...
            match_ind0 = binsearch(int_basis, node_nlocal, new_int0);
            if(match_ind0 == -1){
              cont.push_back(new_int0);
              st.push_back(state);
              continue;
  ...
  // Establish a new group which includes the rank 0 of every node
  MPI_Group petsc_group;
  MPI_Comm_group(PETSC_COMM_WORLD, &petsc_group);
  ...
  MPI_Comm zeronode_comm;
  MPI_Comm_create_group(PETSC_COMM_WORLD, zero_of_node_group, 0, &zeronode_comm);

  // Collective communication between ranks 0 of every node to find global indices
  LLInt *start_node_inds = NULL;
  if(node_rank_ == 0){
    start_node_inds = new LLInt[nodes];
  ...
  if(mpirank_ == 0) basis_help_size = node_nlocal;

  if(node_rank_ == 0) MPI_Bcast(&basis_help_size, 1, MPI_LONG_LONG, 0, zeronode_comm);

  // Create basis_help buffers and initialize them to zero
  LLInt *basis_help = NULL;
  if(node_rank_ == 0){   
    basis_help = new LLInt[basis_help_size];
  ...
  if(node_rank_ == 0){
    for(LLInt i = 0; i < node_nlocal; ++i)
      basis_help[i + (basis_help_size - node_nlocal)] = int_basis[i];
  }

  // Main communication procedure. Each processor within a given node communicates with 
  // it's local rank 0 of node, then each rank 0 does a ring exchange for the rest of 
  // values of the basis
  ...
  if(node_rank_ == 0) recv_sizes = new LLInt[node_size_];

  MPI_Gather(&cont_size, 1, MPI_LONG_LONG, recv_sizes, 1, MPI_LONG_LONG, 0, node_comm_);  
      ...
      for(LLInt i = 0; i < cont_size; ++i){
        if(cont[i] > 0){
          ...
          if(m_ind != -1){
            ...
            cont[i] = -1ULL * (m_ind + start_node_inds[source]);
          }
        }
      }
      ...
      for(PetscMPIInt i = 1; i < node_size_; ++i){
        ...
        for(LLInt ii = 0; ii < rsize; ++ii){
          if(cont_help[ii] > 0){
            ...
            if(m_ind != -1){
              if(basis_help[0] == 0) m_ind = m_ind - 1;
              cont_help[ii] = -1ULL * (m_ind + start_node_inds[source]);
            }
          }
        }
  ...
  delete [] recv_sizes;
  delete [] ranks;
  delete [] start_node_inds;
  delete [] basis_help;
  MPI_Group_free(&petsc_group);
  MPI_Group_free(&zero_of_node_group);
  MPI_Comm_free(&zeronode_comm);
...
void SparseHamiltonian::construct_hamiltonian_matrix(LLInt *int_basis, 
    double V, double t, PetscInt nlocal, PetscInt start, PetscInt end,
        PetscInt node_nlocal, PetscInt node_start, PetscInt node_end)
{
  ...
  this->determine_allocation_details_(int_basis, cont, st, nlocal, start, end, 
    node_nlocal, node_start, node_end, d_nnz, o_nnz);
