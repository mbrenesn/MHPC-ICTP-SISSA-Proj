...
int main(int argc, char **argv)
{
  ...
  PetscInt node_nlocal, node_start, node_end;

  sparse_hamiltonian.distribution(nlocal, start, end);

  // The following distribution is called only by rank 0 of each node,
  // therefore, it's variables only make sense by these processes
  sparse_hamiltonian.node_distribution(node_nlocal, node_start, node_end);

  // Let's construct the int basis that is required to construct the Hamiltonian matrix
  // Each processor creates and holds a distributed section of the integer basis
  // For this version, each rank 0 of every node holds a distributed basis among nodes 
  PetscInt basis_local = nlocal;
  if(node_rank == 0){
    basis_local = node_nlocal;
  }
  ...
  // Neel initial index. Returns the index on the processor that found it, returns -1 
  // for all the other processes. The index is global.
  LLInt neel_index;
  sparse_hamiltonian.get_neel_index(neel_index, int_basis, basis_local, basis_start);
  ...
  sparse_hamiltonian.construct_hamiltonian_matrix(int_basis,V,t,nlocal,start,end,
    node_nlocal,node_start,node_end);
  PetscTime(&constt2);
