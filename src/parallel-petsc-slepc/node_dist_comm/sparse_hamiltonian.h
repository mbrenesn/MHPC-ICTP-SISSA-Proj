...
class SparseHamiltonian
{
  private:
    ...
    void gather_nonlocal_values_(LLInt *start_inds, PetscInt start);
    void determine_allocation_details_(LLInt *int_basis, std::vector<LLInt> &cont,
        std::vector<LLInt> &st, const PetscInt m, PetscInt start, PetscInt end, 
  ...
  public:
    ...
    void node_distribution(PetscInt &nlocal, PetscInt &start, PetscInt &end);
    ...
    void get_neel_index(LLInt &index, LLInt *int_basis, PetscInt n, PetscInt start);
    void get_random_initial_pick(LLInt &pick_ind, LLInt *int_basis, PetscInt start, 
        PetscInt end,  bool wtime, bool verbose);
