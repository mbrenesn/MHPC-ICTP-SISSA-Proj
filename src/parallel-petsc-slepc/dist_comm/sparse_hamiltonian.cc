...
/*******************************************************************************/
void SparseHamiltonian::gather_nonlocal_values_(LLInt *start_inds, PetscInt start)
{
...
void SparseHamiltonian::determine_allocation_details_(LLInt *int_basis, 
    std::vector<LLInt> &cont, std::vector<LLInt> &st, const PetscInt m, PetscInt start, 
        PetscInt end, PetscInt *diag, PetscInt *off)
{
  ...
  gather_nonlocal_values_(start_inds, start);
  ...
  if(mpirank_ == 0) basis_help_size = m;
  ...
  for(LLInt i = 0; i < m; ++i)
    basis_help[i + (basis_help_size - m)] = int_basis[i];
