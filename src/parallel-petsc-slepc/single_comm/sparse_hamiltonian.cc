    ...
    const PetscInt m, PetscInt start, PetscInt end, PetscInt *diag, PetscInt *off)
{
  ...
  // Communication to rank 0 to find size of buffers
  if(mpirank_){
    LLInt cont_size = cont.size(); 
    MPI_Send(&cont_size, 1, MPI_LONG_LONG, 0, mpirank_, PETSC_COMM_WORLD);
  }
  else{
    for(PetscMPIInt i = 1; i < mpisize_; ++i)
      MPI_Recv(&recv_sizes[i - 1], 1, MPI_LONG_LONG, i, MPI_ANY_TAG, PETSC_COMM_WORLD, 
       MPI_STATUS_IGNORE);
  }

  // Communication to rank 0 to find missing indices
  if(mpirank_){
    MPI_Send(&cont[0], cont.size(), MPI_LONG_LONG, 0, mpirank_, PETSC_COMM_WORLD);
    MPI_Recv(&cont[0], cont.size(), MPI_LONG_LONG, 0, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
  }
  else{
    for(PetscMPIInt i = 1; i < mpisize_; ++i){
      MPI_Status stat;
      ...
      cont.resize(rsize);
      MPI_Recv(&cont[0], recv_sizes[i - 1], MPI_LONG_LONG, i, i, PETSC_COMM_WORLD,
        &stat);
      ...
      MPI_Send(&cont[0], recv_sizes[i - 1], MPI_LONG_LONG, stat.MPI_SOURCE, 0, PETSC_COMM_WORLD);
    ...
    double V, double t, PetscInt nlocal, PetscInt start, PetscInt end)
{
  ...
  if(mpirank_ == 0) recv_sizes = new LLInt[mpisize_ - 1];
